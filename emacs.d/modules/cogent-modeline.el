...
(defvar cogent-line-selected-windows (list (cons (selected-frame) (frame-selected-window))))
(defun cogent/set-focused (frame window)
  (setf (alist-get frame cogent-line-selected-windows nil nil #'equal)
        window))
(defun cogent/set-unfocused (frame)
  (setf (alist-get frame cogent-line-selected-windows nil t #'equal) nil))
(defun cogent-line-set-selected-window (&rest _args)
  (let ((cur-win (frame-selected-window)))
    (when (not (minibuffer-window-active-p cur-win))
      (cogent/set-focused (selected-frame) cur-win)
      (force-mode-line-update))))
(defvar cogent-modeline/focus-event-timer nil)
(defun cogent/change-of-focus-function ()
  (dolist (frame (frame-list))
    (pcase (frame-focus-state frame)
      (`t (unless (minibuffer-window-active-p (frame-selected-window frame))
            (cogent/set-focused frame (frame-selected-window frame))))
      (`nil (cogent/set-unfocused frame))))
  (setq cogent-modeline/focus-event-timer nil))
(defun cogent/run-after-focus-change-timer ()
  (unless (timerp cogent-modeline/focus-event-timer)
    (setq cogent-modeline/focus-event-timer
          (run-at-time "0.05 sec" nil #'cogent/change-of-focus-function))))
(add-hook 'window-configuration-change-hook #'cogent-line-set-selected-window)
(add-function :after after-focus-change-function #'cogent/run-after-focus-change-timer)
...
(defun cogent-line-selected-window-active-p ()
  (cl-member (selected-window) (mapcar #'cdr cogent-line-selected-windows) :test #'equal))
